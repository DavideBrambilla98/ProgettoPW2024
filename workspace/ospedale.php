    ...
    <body>
        ...
        <div id="research">      
            <form name="researchForm" method="POST">
                ...
                <input id="direttoreSanitario" name="direttoreSanitario" type="text" placeholder="direttore sanitario"/>
                <button type="submit">
            ...
            <?php
                    ...
                    $statoPDO->execute();
                        ...
                        if ($statoPDO->rowCount() > 0) {
                            echo "<table><tr><th>Codice struttura</th><th>Nome</th><th>Indirizzo</th><th>Comune</th><th>Direttore sanitario</th></tr>";
                            // stampa i dati di ogni riga
                            while($row = $statoPDO->fetch()) {
                                echo "<tr><td>".$row["CodiceStruttura"]."</td><td>".$row["DenominazioneStruttura"]."</td><td>".$row["Indirizzo"]."</td><td>".$row["Comune"]."</td><td>".$row["DirettoreSanitario"]."</td></tr>";
                            }
                            echo "</table>";
                        } else {
                            echo "0 results";
